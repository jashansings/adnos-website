...
const ThemeTransition = ({ targetTheme }: { targetTheme: 'dark' | 'light' | null }) => {
  return (
    <AnimatePresence>
      {targetTheme && (
        <div className="fixed inset-0 z-[100000] pointer-events-none flex flex-col">
          ...
          {[...Array(5)].map((_, i) => (
            <motion.div
              key={i}
              className={`w-full flex-1 ${targetTheme === 'dark' ? 'bg-black' : 'bg-adnos-offwhite'}`}
              initial={{ scaleX: 0, transformOrigin: i % 2 === 0 ? 'left' : 'right' }}
              animate={{ scaleX: 1 }}
              exit={{ scaleX: 0, transformOrigin: i % 2 === 0 ? 'right' : 'left' }}
              transition={{ duration: 0.6, delay: i * 0.08, ease: [0.85, 0, 0.15, 1] }}
            />
          ))}
          ...
          >
            <div className="relative w-full h-full max-w-4xl">
              {targetTheme === 'light' ? (
                /* --- EARLY BIRD ANIMATION --- */
                <>
                  {/* Rising Black Sun */}
                  <motion.div 
                    className="absolute left-1/2 -translate-x-1/2 bg-black rounded-full shadow-[0_0_60px_rgba(0,0,0,0.3)]"
                    style={{ width: '250px', height: '250px', bottom: '-50px' }}
                    initial={{ y: 200 }}
                    animate={{ y: 0 }}
                    transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
                  />
                  {/* Leading Bird */}
                  <motion.svg className="absolute w-32 h-32" style={{ top: '30%', left: '40%' }} viewBox="0 0 100 100"
                    initial={{ x: -150, y: 100, scale: 0.5, rotate: -15 }}
                    animate={{ x: 0, y: 0, scale: 1, rotate: 0 }}
                    transition={{ duration: 0.9, ease: "easeOut" }}
                  >
                    <path d="M10 50 Q25 30 50 50 Q75 30 90 50" fill="none" stroke="black" strokeWidth="5" strokeLinecap="round" />
                  </motion.svg>
                  {/* Trailing Bird */}
                  <motion.svg className="absolute w-20 h-20" style={{ top: '45%', left: '25%' }} viewBox="0 0 100 100"
                    initial={{ x: -150, y: 100, scale: 0.5, rotate: -20 }}
                    animate={{ x: 0, y: 0, scale: 1, rotate: 0 }}
                ...
                <>
                  {/* White Crescent Moon */}
                  <motion.svg className="absolute w-24 h-24 md:w-32 md:h-32" style={{ top: '15%', right: '15%' }} viewBox="0 0 100 100"
                    initial={{ scale: 0, rotate: -45 }}
                    animate={{ scale: 1, rotate: 0 }}
                    transition={{ duration: 0.8, ease: "easeOut" }}
                  >
                    <path d="M55 10 A40 40 0 1 0 90 65 A35 35 0 1 1 55 10 Z" fill="white" />
                  </motion.svg>

                  {/* Twinkling Stars */}
                  {[
                    { t: '15%', l: '20%', d: 0.1, s: '12px' },
                    { t: '30%', l: '45%', d: 0.3, s: '8px' },
                    { t: '20%', l: '60%', d: 0.5, s: '10px' },
                    { t: '45%', l: '15%', d: 0.4, s: '6px' },
                    { t: '40%', l: '80%', d: 0.2, s: '14px' },
                  ].map((star, i) => (
                    <motion.div 
                      key={i} 
                      ...
                      transition={{ duration: 1.5, delay: star.d }} 
                    />
                  ))}

                  {/* White Pine Tree */}
                  <motion.svg className="absolute w-40 h-64 md:w-56 md:h-80" style={{ bottom: '-20px', left: '10%' }} viewBox="0 0 100 100" preserveAspectRatio="none"
                    initial={{ y: 200 }} 
                    animate={{ y: 0 }} 
                    transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
                  >
                    <path d="M50 10 L100 100 L0 100 Z" fill="white" />
                    <path d="M50 30 L90 100 L10 100 Z" fill="white" opacity="0.8" />
                  </motion.svg>

                  {/* White Owl with Black Accents */}
                  <motion.svg className="absolute w-40 h-40 md:w-56 md:h-56" style={{ bottom: '-10px', right: '15%' }} viewBox="0 0 100 100"
                    initial={{ y: 150, opacity: 0 }} 
                    animate={{ y: 0, opacity: 1 }} 
                    transition={{ duration: 0.8, delay: 0.1, ease: [0.16, 1, 0.3, 1] }}
                  >
                    {/* Owl Base/Body */}
                    <path d="M20 100 L20 50 Q20 20 50 20 Q80 20 80 50 L80 100 Z" fill="white" />
                    {/* Owl Horns/Ears */}
                    <path d="M20 40 L10 10 L45 30 M80 40 L90 10 L55 30" fill="white" />
                    {/* Black Eyes */}
                    <circle cx="35" cy="50" r="14" fill="black" />
                    <circle cx="65" cy="50" r="14" fill="black" />
                    {/* White Pupils */}
                    <circle cx="35" cy="50" r="4" fill="white" />
                    <circle cx="65" cy="50" r="4" fill="white" />
                    {/* Black Beak */}
                    <path d="M50 62 L42 78 L58 78 Z" fill="black" />
                  </motion.svg>
                </>
              )}
            </div>
...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-[100] px-6 md:px-12 py-4 md:py-6 flex justify-between items-center backdrop-blur-md border-b transition-colors duration-0 ${isDark ? 'bg-black/80 border-white/10' : 'bg-adnos-offwhite/80 border-black/5'}`}>
      <div className="flex items-center gap-4">
        <a className="flex items-center gap-4 group" href="#hero">
          <img
            src="/logo.png"
            alt="ADNOS Logo"
            className={`h-10 md:h-12 w-auto object-contain transition-transform group-hover:rotate-6 ${isDark ? 'invert' : ''}`}
          />
          <div className={`mono-label font-black text-[10px] md:text-xs hidden sm:flex border px-3 py-1.5 items-center gap-2 cursor-pointer transition-colors ${isDark ? 'bg-black text-white border-white/20 hover:border-glitch-red' : 'bg-white text-adnos-black border-black hover:border-glitch-red'}`}>
            ADNOS STUDIOS
          </div>
        </a>
      </div>
      <div className={`flex items-center gap-4 md:gap-10 mono-label text-[10px] md:text-xs ${isDark ? 'text-white' : 'text-adnos-black'}`}>
        <a className="hover:text-glitch-red transition-colors hidden md:block" href="#hero">HOME</a>
        <a className="hover:text-glitch-red transition-colors hidden md:block" href="#vault">WORK</a>
        <a className="hover:text-glitch-red transition-colors hidden md:block" href="#about-team">ABOUT</a>
        <a className="hover:text-glitch-red transition-colors hidden md:block" href="#contact">CONTACT</a>
        
        {/* The Theme Toggle Button */}
        <button 
          onClick={toggleTheme} 
          className={`ml-2 p-2.5 rounded-full border flex items-center justify-center transition-all ${isDark ? 'border-white/20 hover:bg-white hover:text-black' : 'border-black hover:bg-black hover:text-white'}`}
...
const Hero = () => {
  ...
  return (
    <section className={`flex flex-col items-center justify-center relative overflow-hidden px-6 pt-40 min-h-[120vh] pb-24 transition-colors duration-0 ${isDark ? 'bg-[#050505] text-white' : 'bg-adnos-offwhite text-adnos-black'}`} id="hero">
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-5">
        <svg className="w-[200vw] md:w-[180vw] h-auto animate-rotate-slow" viewBox="0 0 500 500">
          ...
          <text className={`font-black text-[22px] uppercase font-montserrat ${isDark ? 'fill-white' : 'fill-adnos-black'}`}>
      ...
      <div className="relative z-20 text-center max-w-5xl">
        ...
        <Reveal delay={0.2}>
          <p className={`font-black text-2xl md:text-6xl lg:text-7xl leading-[0.9] uppercase max-w-5xl mx-auto font-montserrat ${isDark ? 'text-white' : 'text-adnos-black'}`}>
        ...
        <Reveal delay={0.6}>
          <div className="mt-12 md:mt-16 flex flex-col items-center">
            ...
            <div className={`w-px h-16 md:h-24 ${isDark ? 'bg-white/20' : 'bg-black/20'}`}></div>
          </div>
        </Reveal>
      </div>
    </section>
...
const Showreel = () => {
  ...
  // Showreel section is natively dark, it doesn't need to invert.
  return (
    <section className="bg-adnos-black py-20 md:py-32 px-6 relative" id="showreel">
      <div className="max-w-[1400px] mx-auto">
        ...
        <Reveal>
          <div 
            onClick={togglePlay}
            className="relative group border-[1px] aspect-video overflow-hidden bg-black mx-auto shadow-[0_0_50px_rgba(255,255,255,0.1)] border-white/20 md:border-white/40 border md:border-2 cursor-pointer"
          >
    ...
    <div ref={scrollRef} className="relative h-[250vh]">
      <div className="sticky top-0 h-screen overflow-hidden flex items-center">
        <motion.div style={{ x }} className="flex w-[200vw]">
          {/* PANEL 1: THE TEAM */}
          <section className={`w-screen h-screen flex-shrink-0 flex items-center px-6 md:px-24 transition-colors duration-0 ${isDark ? 'bg-[#0a0a0a] text-white' : 'bg-adnos-offwhite text-adnos-black'}`}>
            <div className="max-w-[1400px] w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
              <div>
                ...
                <div className="space-y-6 text-sm md:text-xl font-mono leading-relaxed max-w-xl opacity-80">
              ...
              <div className="hidden lg:flex justify-center items-center">
                <div className={`w-full max-w-md aspect-square border-[20px] flex items-center justify-center p-10 shadow-[20px_20px_0px_#ff003c] hover:shadow-[20px_20px_0px_#003cff] transition-all duration-500 ${isDark ? 'border-white/10 bg-[#111]' : 'border-black bg-white'}`}>
                  <img 
                    alt="Core Concept" 
                    className={`w-full h-full object-contain grayscale hover:grayscale-0 transition-all duration-700 ${isDark ? 'invert' : ''}`} 
    ...
    <section className={`py-24 md:py-32 px-6 relative z-40 border-t transition-colors duration-0 ${isDark ? 'bg-[#050505] border-white/20' : 'bg-adnos-offwhite border-black'}`} id="vault">
      <div className="max-w-[1400px] mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-baseline mb-12 md:mb-16">
          <Reveal>
            <h2 
              className={`display-brutalist text-section-title hover:text-glitch-red transition-colors mb-4 md:mb-8 cursor-pointer ${isDark ? 'text-white' : 'text-adnos-black'}`}
              onClick={triggerSystemGlitch}
            >
              PORTFOLIO
            </h2>
        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {showreels.map((reel) => (
              <div 
                key={reel.id} 
                className={`relative aspect-video bg-black border overflow-hidden group hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all ${isDark ? 'border-white/20 shadow-[5px_5px_0px_rgba(255,255,255,0.1)]' : 'border-black shadow-[5px_5px_0px_rgba(0,0,0,1)]'}`}
              >
        ...
        <div className={`mb-8 flex items-center justify-between border-b pb-4 ${isDark ? 'border-white/20 text-white' : 'border-black text-adnos-black'}`}>
          <MonoLabel className="text-sm font-bold">Selected Projects</MonoLabel>
        </div>
        
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border-t border-l bg-black ${isDark ? 'border-white/20' : 'border-black'}`}>
          {projects.map((item) => (
            <div key={item.id} className={`aspect-[4/5] bg-black overflow-hidden relative group border-r border-b ${isDark ? 'border-white/20' : 'border-black'}`}>
        ...
        <div className="mt-16 md:mt-20 flex justify-center">
          <button className={`mono-label px-8 md:px-12 py-4 md:py-5 border transition-all text-[10px] md:text-xs ${isDark ? 'border-white text-white hover:bg-white hover:text-black' : 'border-black hover:bg-black hover:text-white'}`}>
            SEE THE FULL CHAOS
          </button>
        </div>
      </div>
...
const Crew = () => {
  ...
  return (
    <section className={`py-24 md:py-32 px-6 border-t relative z-30 transition-colors duration-0 ${isDark ? 'bg-[#0a0a0a] border-white/20' : 'bg-adnos-offwhite border-black'}`} id="about-team">
      <div className="max-w-[1400px] mx-auto">
        <div className="mb-12 md:mb-16">
          <MonoLabel className="text-glitch-red mb-4">BEHIND THE LENS</MonoLabel>
          <Reveal>
            <h2 
              className={`display-brutalist text-section-title hover:text-glitch-red transition-colors cursor-pointer ${isDark ? 'text-white' : 'text-adnos-black'}`}
              onClick={triggerSystemGlitch}
            >
              OUR CREW
            </h2>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border-t border-l overflow-hidden ${isDark ? 'border-white/20' : 'border-black'}`}>
          {[
            ...
            <div key={member.name} className={`p-6 md:p-8 group transition-all border-r border-b ${isDark ? 'hover:bg-white hover:text-black border-white/20 text-white' : 'bg-adnos-offwhite hover:bg-black hover:text-white border-black text-adnos-black'}`}>
              <div className={`aspect-square mb-6 overflow-hidden border relative ${isDark ? 'bg-[#111] border-white/20' : 'bg-adnos-gray border-black'}`}>
                ...
                />
                <div className={`absolute top-4 right-4 font-black text-2xl transition-colors pointer-events-none drop-shadow-md z-10 ${isDark ? 'text-white/20 group-hover:text-black/40' : 'text-white/30 group-hover:text-white/60'}`}>
                  {member.id}
                </div>
              </div>
...
export default function App() {
  ...
  const toggleTheme = () => {
    if (targetTheme) return; // Prevent clicking while already transitioning
    
    const nextTheme = isDark ? 'light' : 'dark';
    setTargetTheme(nextTheme);

    // Swap the actual UI theme exactly when the shutters fully cover the screen
    setTimeout(() => {
      setIsDark(!isDark);
    }, 800);

    // Remove the shutters after the theme has successfully swapped underneath
    setTimeout(() => {
      setTargetTheme(null);
    }, 1500);
  };
  ...
  return (
    <ThemeContext.Provider value={{ isDark, toggleTheme }}>
      <main className="relative">
        ...
        <ThemeTransition targetTheme={targetTheme} />
      </main>
